    ...
    /// Default configuration for `no-static-element-interactions`.
    ///
    /// This lists the default `handlers` used when no configuration is provided.
    /// Generated docs rely on these comments to show default values for flattened
    /// or optional config fields.
    fn default_no_static_element_interactions_handlers() -> serde_json::Value {
        serde_json::json!({
